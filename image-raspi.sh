...
then
    ...
    # If the user successfully passes the failsafe, then create an image
    # Unsure if unmounting is necessary, might as well have it just in case
    if [[ $imageConfirmation == $failsafe ]]; then
        echo "Imaging from " ~/images/$imagePath " to " /dev/$devName
        umount /dev/$devName* -f -l
...
then
    ...
    fdisk -l
    echo ""
    echo "Please type in each device name, then type 'complete' when done"
    declare -a physicalDiskPath
    while read diskPath
    do
        i=i+1
        read physicalDiskPath=$("${diskPath[@]}")
        if [[ diskPath[$i]==complete ]]
        then
            physicalDiskPath=("${array[@]/complete}")
            break
        fi
    done

    # TODO: add support for multiple destination disks. Currently, only one disk, so overriding user's input.
    diskCount = ${#physicalDiskPath[@]}
    echo "We counted "$diskCount" disks"
    ...
    for i in $diskCount
    do
        echo "You're imaging disk /dev/$physicalDiskPath[$i]"
    done
